...
Como se estableció en la sección anterior, la lógica de primer orden proporciona la estructura representacional base. Sin embargo, para modelar procesos estocásticos es imperativo cuantificar la incertidumbre. La Programación Lógica Probabilística (PLP) extiende la programación clásica con esta capacidad. Específicamente, la \textit{semántica de distribución} define el marco probabilístico bajo el cual opera el motor de inferencia de MDP-ProbLog y constituye la base formal para el cálculo exacto de las probabilidades de transición entre estados.
...
De esta manera, la sustitución de evidencia en el circuito compilado es matemáticamente equivalente a calcular la probabilidad condicional $P(x'_i \mid s, a)$ bajo la semántica de distribución, sin necesidad de aplicar explícitamente la regla de Bayes. Esta equivalencia, demostrada formalmente por Bueno \textit{et al.} \cite{bueno2016mdp}, es lo que permite que cada evaluación del programa ProbLog corresponda a un \textit{backup} de Bellman.


Aunque la semántica de distribución proporciona el marco teórico impecable para definir estas probabilidades, el cálculo directo sumando sobre mundos posibles crece exponencialmente con el número de hechos. Para resolver este desafío en la práctica, el problema se transforma utilizando técnicas de evaluación de fórmulas booleanas, concepto que se detalla a continuación.
...
Para materializar la inferencia probabilística en tiempos computacionalmente tratables, ProbLog recurre al Conteo de Modelos Ponderados (WMC, por sus siglas en inglés). El WMC es la operación computacional central que MDP-ProbLog ejecuta de forma iterativa para calcular las probabilidades de transición. El cálculo directo de la Ecuación~\ref{eq:success-probability} requiere enumerar exponencialmente muchos mundos posibles, lo cual es computacionalmente intratable en general.
...
Con las herramientas lógicas (para la representación del conocimiento) y el WMC (para la inferencia tratable) plenamente definidas, el marco de trabajo cuenta con los mecanismos necesarios para evaluar el estado estático de un sistema. El siguiente capítulo aborda cómo modelar agentes que interactúan y modifican su entorno de manera temporal mediante los Procesos de Decisión de Markov.
...
Los Procesos de Decisión de Markov (MDPs) formalizan el problema de decisión secuencial estocástica que la arquitectura de MDP-ProbLog está diseñada para resolver. Hasta este punto, se han descrito exclusivamente las herramientas lógicas y probabilísticas para evaluar el estado estático de un sistema. Sin embargo, para modelar agentes que interactúan, toman decisiones y modifican su entorno a lo largo del tiempo, es necesario incorporar este paradigma matemático. 
...
[Agregar una representación visual de el grid y la codificación de los fluentes para ilustrar este punto.]
[Citar y mencionar si es necesario de donde vienen los modelos de grids]
...
MDP-ProbLog es un framework que combina la expresividad de ProbLog con la resolución de MDPs de horizonte infinito \cite{bueno2016mdp}. 
...
ProbLog utiliza los d-DNNF como su estructura de compilación principal \cite{vlasselaer2016problog2}.
...
El impacto real de esta transición representacional sobre el tamaño de los circuitos generados y, por consiguiente, en los tiempos de inferencia computacional, constituye una de las hipótesis centrales de esta investigación. Este comportamiento se analizará y cuantificará empíricamente en el Capítulo de Experimentación y Resultados.
