...
En el capítulo anterior se estableció que MDP-ProbLog carece de soporte nativo para variables de estado multivaluadas, y que las Disyunciones Anotadas constituyen un mecanismo candidato para resolver esta limitación. El presente capítulo examina los antecedentes científicos que sustentan esta propuesta desde tres ejes: la representación factorizada de MDPs y el problema de la binarización forzada de variables, el uso de la Programación Lógica Probabilística como lenguaje de especificación para la toma de decisiones secuenciales, y los mecanismos de exclusión mutua que permiten la inferencia eficiente. El análisis de estos trabajos permite identificar el vacío de investigación que la presente tesina aborda.
...
Dado que MDP-ProbLog está construido sobre el lenguaje Prolog, es necesario establecer los fundamentos de la programación lógica que sustentan tanto la representación del conocimiento como el proceso de inferencia del sistema. La programación lógica es un paradigma de computación donde los programas se expresan como conjuntos de sentencias lógicas y la ejecución consiste en realizar inferencias sobre dichas sentencias.

El vocabulario de un programa lógico se construye a partir de \textit{constantes} (valores concretos como \texttt{rojo}, \texttt{a}), \textit{variables} (denotadas con mayúscula inicial, como \texttt{X}) y \textit{functores} (nombres de predicados con aridad asociada). Un \textit{término} es una constante, variable o functor aplicado a términos; un \textit{átomo} es un predicado aplicado a sus argumentos (e.g., \texttt{color(carro, rojo)}); y un \textit{literal} es un átomo o su negación. A partir de estos elementos se construyen las cláusulas que componen un programa lógico.
...
La semántica de distribución define el marco probabilístico bajo el cual opera el motor de inferencia de MDP-ProbLog y constituye la base formal para el cálculo de las probabilidades de transición entre estados. La Programación Lógica Probabilística (PLP) extiende la programación lógica con la capacidad de manejar incertidumbre de manera formal.
