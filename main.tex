...
Probabilistic Logic Programming (PLP) is a paradigm that combines the representational capacity of first-order logic with probability theory to manage uncertainty. This paradigm enables the modeling of a wide variety of domains on real-world situations in which complex interactions exist among sets of elements and varying levels of uncertainty. Within this domain, the MDP-ProbLog framework utilizes the capabilities of the ProbLog language for the specification and resolution of infinite-horizon Markov Decision Processes (MDPs) through a factored, logical, and relational representation.

However, the base architecture of this system presents a structural limitation, as the representation of state fluents is restricted exclusively to independent Bernoulli variables (Boolean). As a result, to describe variables that are inherently multi-valued, the user is compelled to employ techniques for decomposing them into multiple binary predicates. In the case of MDP-ProbLog, this restriction leads to a tedious, impractical modeling process that is prone to semantic inconsistencies.

The development of an extension to the core of the MDP-ProbLog framework is reported in order to resolve this issue. The work includes a review of the source-code architecture, the formulation and design of the extension, the programming, the execution of tests, and the corresponding documentation. Through modifications to the state-management modules and the inference engine, support for multi-valued state fluents was integrated by means of the native mechanism of Annotated Disjunctions. As a result of this development, the modeling of MDPs that require variables with multi-valued domains becomes a semantically integral process that is far more direct and natural for the user.
